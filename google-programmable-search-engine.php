...
class WP_Google_Search {
	...
	public function init() {
		...
		load_plugin_textdomain( 'google-search-plugin-aman', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		...
		//if ( !empty($options['google_search_engine_id']) ) { //$options['enable_plugin']
			...
			if ( isset($options['use_default_correction_css']) and $options['use_default_correction_css'] == 1 )
				...
				wp_enqueue_style( 'wgs', plugins_url('wgs.css', __FILE__), array(), '1.0.10' ); // Increment to reflect updates
	...
	public function wgs_activation() {
		...
		if ($options['search_gcse_page_id'] == null or get_post($options['search_gcse_page_id']) == null) {
			...
			$search_gcse_page = array(
			  ...
			  'post_title'     => __('Suchergebnisse','google-search-plugin-aman'), //[ <string> ] // The title of your post.
			  ...
			  'post_excerpt'   => __('Search Results','google-search-plugin-aman'), //[ <string> ] // For all your post excerpt needs.
